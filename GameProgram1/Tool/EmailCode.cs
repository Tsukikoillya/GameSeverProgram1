...
namespace GameProgram1.Tool
{
    class EmailCode
    {
        ...
        private string mSenderServerHost= "smtp.163.com";    //发件箱的邮件服务器地址（IP形式或字符串形式均可）
        private string mSenderPassword= "OGKABNWGXQZCBPMC";    //发件箱的密码
        private string mSenderUsername= "ao_study_work";   //发件箱的用户名（即@符号前面的字符串，例如：hello@163.com，用户名为：hello）
        private bool mEnableSsl=true;    //是否对邮件内容进行socket层加密传输
        private bool mEnablePwdAuthentication=false;  //是否对发件人邮箱进行密码验证
        private string fromMail = "ao_study_work@163.com";
